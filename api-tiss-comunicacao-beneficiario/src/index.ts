    ...
    <soapenv:Body>
        <ans:comunicacaoBeneficiarioWS>
            ...
            <ans:comunicacaoBeneficiario>
                ...
                <ans:tipoComunicacao>${tipoComunicacao}</ans:tipoComunicacao>
                <ans:dataComunicacao>${dataComunicacao}</ans:dataComunicacao>
                <ans:mensagem>${mensagem}</ans:mensagem>
                ${dadosComplementares ? `<ans:dadosComplementares>${dadosComplementares}</ans:dadosComplementares>` : ''}
            </ans:comunicacaoBeneficiario>
...
export default {
  async fetch(request, env, ctx) {
    ...
    const paramTipoComunicacao = tipoComunicacao ?? searchParams.get('tipoComunicacao');
    const paramMensagem = mensagem ?? searchParams.get('mensagem');
    ...
    if (!paramTipoComunicacao) return Response.json(
      { success: false, errors: [{ message: "Bad Request: tipoComunicacao is required" }]}, 
      { status: 400 }
    );

    if (!paramMensagem) return Response.json(
      { success: false, errors: [{ message: "Bad Request: mensagem is required" }]}, 
      { status: 400 }
    );
    ...
    const { hashedXml } = extractXmlValuesAndHash(buildXML({
      ...
      nomeBeneficiario: nomeBeneficiario ?? searchParams.get('nomeBeneficiario'),
      tipoComunicacao: paramTipoComunicacao,
      dataComunicacao: dataComunicacao ?? searchParams.get('dataComunicacao') ?? new Date().toISOString().split("T")[0],
      mensagem: paramMensagem,
      dadosComplementares: dadosComplementares ?? searchParams.get('dadosComplementares'),
    }));

    const response = await fetch('https://api.servicos.grupoamil.com.br/api-tiss-comunicacao-beneficiario/v4.01.00', {
      method: 'POST',
      headers: {
