...
const buildXML = ({
  ...
  registroANS,
  senhaPrestador,
  numeroCarteira, codigoProcedimento, numeroGuiaPrincipal, nomeBeneficiario, indicacaoClinica, observacao,
        ...
        <ans:solicitacaoProcedimentoWS>
            ...
            <ans:solicitacaoProcedimento>
                <ans:solicitacaoSP-SADT>
                    ...
                    ${indicacaoClinica ? `<ans:indicacaoClinica>${indicacaoClinica}</ans:indicacaoClinica>` : ''}
...
export default {
  async fetch(request, env, ctx) {
    ...
    const paramCodigoProcedimento = codigoProcedimento ?? searchParams.get('codigoProcedimento');
    if (!paramCodigoProcedimento) return Response.json(
      { success: false, errors: [{ message: "Bad Request: codigoProcedimento is required" }]}, 
      { status: 400 }
    );
    ...
    const { hashedXml } = extractXmlValuesAndHash(buildXML({
      ...
      numeroCarteira: paramNumeroCarteira, codigoProcedimento: paramCodigoProcedimento,
      numeroGuiaPrincipal: numeroGuiaPrincipal ?? searchParams.get('numeroGuiaPrincipal'), nomeBeneficiario: nomeBeneficiario ?? searchParams.get('nomeBeneficiario'), indicacaoClinica: indicacaoClinica ?? searchParams.get('indicacaoClinica'), observacao: observacao ?? searchParams.get('observacao'),
    }));

    const response = await fetch('https://api.servicos.grupoamil.com.br/api-tiss-solicitacao-procedimento/v4.01.00', {
      method: 'POST',
      headers: {
