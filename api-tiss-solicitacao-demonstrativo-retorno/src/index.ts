...
const buildXML = ({
  ...
  registroANS,
  senhaPrestador,
  numeroProtocolo, dataInicio, dataFim,
    ...
    <soapenv:Body>
        <ans:solicitacaoDemonstrativoRetornoWS>
            ...
            <ans:solicitacaoDemonstrativoRetorno>
                ...
                ${dataInicio ? `<ans:dataInicio>${dataInicio}</ans:dataInicio>` : ''}
                ${dataFim ? `<ans:dataFim>${dataFim}</ans:dataFim>` : ''}
            </ans:solicitacaoDemonstrativoRetorno>
...
export default {
  async fetch(request, env, ctx) {
    ...
    const { hashedXml } = extractXmlValuesAndHash(buildXML({
      ...
      numeroProtocolo: paramNumeroProtocolo,
      dataInicio: dataInicio ?? searchParams.get('dataInicio'), dataFim: dataFim ?? searchParams.get('dataFim'),
    }));

    const response = await fetch('https://api.servicos.grupoamil.com.br/api-tiss-solicitacao-demonstrativo-retorno/v4.01.00', {
      method: 'POST',
      headers: {
