    ...
    <soapenv:Body>
        <ans:loteGuiasWS>
            ...
            <ans:loteGuias>
                ...
                ${guias ? `<ans:guias>${guias}</ans:guias>` : ''}
            </ans:loteGuias>
...
export default {
  async fetch(request, env, ctx) {
    ...
    const { hashedXml } = extractXmlValuesAndHash(buildXML({
      ...
      observacoes: observacoes ?? searchParams.get('observacoes'), guias: guias ?? searchParams.get('guias'),
    }));

    const response = await fetch('https://api.servicos.grupoamil.com.br/api-tiss-lote-guias/v4.01.00', {
      method: 'POST',
      headers: {
