    ...
    <soapenv:Body>
        <ans:solicitacaoStatusProtocoloWS>
            ...
            <ans:solicitacaoStatusProtocolo>
                ...
                ${registroANS ? `<ans:registroANS>${registroANS}</ans:registroANS>` : ''}
            </ans:solicitacaoStatusProtocolo>
...
export default {
  async fetch(request, env, ctx) {
    ...
    const { hashedXml } = extractXmlValuesAndHash(buildXML({
      ...
      registroANS: registroANS ?? searchParams.get('registroANS'),
    }));

    const response = await fetch('https://api.servicos.grupoamil.com.br/api-tiss-solicitacao-status-protocolo/v4.01.00', {
      method: 'POST',
      headers: {
