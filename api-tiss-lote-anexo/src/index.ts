    ...
    <soapenv:Body>
        <ans:loteAnexoWS>
            ...
            <ans:loteAnexo>
                ...
                ${anexos ? `<ans:anexos>${anexos}</ans:anexos>` : ''}
            </ans:loteAnexo>
...
export default {
  async fetch(request, env, ctx) {
    ...
    const { hashedXml } = extractXmlValuesAndHash(buildXML({
      ...
      observacoes: observacoes ?? searchParams.get('observacoes'), anexos: anexos ?? searchParams.get('anexos'),
    }));

    const response = await fetch('https://api.servicos.grupoamil.com.br/api-tiss-lote-anexo/v4.01.00', {
      method: 'POST',
      headers: {
