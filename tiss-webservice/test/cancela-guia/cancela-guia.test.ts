...
vi.mock('soap', () => ({
  createClientAsync: vi.fn().mockImplementation((url: string) => {
    return Promise.resolve({
      tissCancelaGuia_OperationAsync: vi.fn().mockImplementation((params, options) => {
...
const ws = new TissWebservice({
  CANCELA_GUIA: 'https://www.tiss.gov.br/tiss/services/CancelaGuiaV40100',
});

describe('cancela-guia', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ans="http://www.ans.gov.br/padroes/tiss/schemas">
  <soap:Body>
    <ans:tissCancelaGuia_Operation>
      <ans:cancelaGuiaWS>
        ...
        <ans:cancelaGuia>
          ...
          <ans:observacao>Cancelamento solicitado pelo médico responsável</ans:observacao>
        </ans:cancelaGuia>
    ...
    expect(response.reciboCancelaGuia?.situacaoCancelamento).toBe('CANCELADA');
    expect(response.reciboCancelaGuia?.numeroGuiaOperadora).toBe('OP20240115001');
