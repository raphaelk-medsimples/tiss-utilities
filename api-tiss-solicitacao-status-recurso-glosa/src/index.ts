    ...
    <soapenv:Body>
        <ans:solicitacaoStatusRecursoGlosaWS>
            ...
            <ans:solicitacaoStatusRecursoGlosa>
                ...
                ${numeroLote ? `<ans:numeroLote>${numeroLote}</ans:numeroLote>` : ''}
            </ans:solicitacaoStatusRecursoGlosa>
...
export default {
  async fetch(request, env, ctx) {
    ...
    const { hashedXml } = extractXmlValuesAndHash(buildXML({
      ...
      numeroLote: numeroLote ?? searchParams.get('numeroLote'),
    }));

    const response = await fetch('https://api.servicos.grupoamil.com.br/api-tiss-solicitacao-status-recurso-glosa/v4.01.00', {
      method: 'POST',
      headers: {
